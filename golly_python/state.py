...
class BinaryLifeState(LifeState):
    ...
    def __init__(self, state1: LifeList, state2: LifeList):
        ...
        self.statelist = LifeList()
        self.statelist1 = state1
        self.statelist2 = state2

        if self.statelist1.rows != self.statelist2.rows:
            err = "Error: CompositeLifeState received states of different sizes:\n"
            err += f"state 1 rows {self.statelist1.rows} != state 2 rows {self.statelist2.rows}"
            raise Exception(err)
        self.rows = self.statelist1.rows

        if self.statelist1.columns != self.statelist2.columns:
            err = "Error: CompositeLifeState received states of different sizes:\n"
            err += f"state 1 columns {self.statelist1.columns} != state 2 columns {self.statelist2.columns}"
            raise Exception(err)
        self.columns = self.statelist1.columns

        if (
            self.statelist1.neighbor_color_legacy_mode
            != self.statelist2.neighbor_color_legacy_mode
        ):
            err = "Error: CompositeLifeState received states with different neighbor_color_legacy_mode settings"
            raise Exception(err)
        self.neighbor_color_legacy_mode = self.statelist1.neighbor_color_legacy_mode
